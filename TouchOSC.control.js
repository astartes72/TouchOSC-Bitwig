...
function init()
{
   ...
   tOSC.userControls = host.createUserControls((tOSC.HIGHEST_CC - tOSC.LOWEST_CC + 1)*16);
   ...
   for(var i=tOSC.LOWEST_CC; i<=tOSC.HIGHEST_CC; i++)
   {
			for (var j=1; j<=16; j++) {
				 ...
				 tOSC.userControls.getControl(c).setLabel("CC " + i + " - Channel " + j);
				 // Add a ValueObserver for each userControl
	       tOSC.userControls.getControl(c).addValueObserver(128, getValueObserverFunc(c, tOSC.ccValue));
			}
   }
}
   ...
   if (isChannelController(status))
   {
      if (data1 >= tOSC.LOWEST_CC && data1 <= tOSC.HIGHEST_CC)
      {
         ...
         tOSC.userControls.getControl(index).set(data2, 128);
			...
			else if (data1 >= tOSC.FADERS && data1 < tOSC.FADERS + 9 ) {
